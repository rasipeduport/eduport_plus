...
export default function AdminsPage() {
  ...
  const openModal = (type, admin) => {
    ...
    if (type === 'edit_details') {
      setFullNameVal(admin.full_name || '');
      setMobileNumberVal(admin.mobile_number || '');
    } else if (type === 'edit_email') {
      setEditEmailVal(admin.email);
    }
  };
  ...
  const handleEditDetails = async (e) => {
    ...
    setModalError('');
    try {
      await api.patch(`/api/users/${activeAdmin.id}/`, {
      ...
      setModalError(err.response?.data?.message || 'Failed to update admin details.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteUser = async () => {
    ...
    setModalError('');
    try {
      await api.delete(`/api/users/${activeAdmin.id}/`);
      fetchAdmins();
      closeModal();
    } catch (err) {
          ...
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody>
              {filteredAdmins.length === 0 ? (
                ...
                getSortedData(filteredAdmins).map(admin => {
                  const initials = admin.full_name?.split(' ').map(n => n[0]).join('').substring(0, 2) || 'A';
                  return (
                    <tr
                      key={admin.id}
                    ...
                    >
                      ...
                      {visibleColumns.full_name && (
                        <td className="py-2 px-6 font-medium text-white text-sm align-middle whitespace-nowrap">
                          {admin.kind === 'ghost' ? (
                            ...
                            admin.full_name || <span className="text-zinc-500">—</span>
                          )}
                        </td>
                      )}
                      {visibleColumns.email && (
                        <td className="py-2 px-6 text-white text-sm align-middle whitespace-nowrap">{admin.email}</td>
                      )}
                      {visibleColumns.mobile_number && (
                        <td className="py-2 px-6 text-[#a1a1aa] text-sm align-middle whitespace-nowrap">
                          {admin.kind === 'ghost' ? '—' : (admin.mobile_number || '—')}
                        </td>
                      )}
                      {visibleColumns.created_at && (
                        <td className="py-2 px-6 text-[#a1a1aa] text-sm align-middle whitespace-nowrap">
                          {admin.kind === 'ghost' ? '—' : new Date(admin.created_at).toLocaleDateString('en-US', {
                      ...
                      {visibleColumns.invited_by && (
                        <td className="py-2 px-6 text-zinc-300 text-sm align-middle whitespace-nowrap">
                          {admin.invited_by_profile?.full_name || admin.invited_by_profile?.email || '—'}
                        </td>
                      )}
                      <td className="py-2 px-2 align-middle text-right sticky right-0 bg-[#0a0a0a] group-hover:bg-[#111] border-l border-[rgba(255,255,255,0.08)] transition-colors z-10">
                        {currentUser?.role === 'ADMIN' && (
                          <StaffActionsDropdown
                            items={admin.kind === 'active' ? [
                              { label: 'Edit Details', onClick: () => openModal('edit_details', admin) },
                              { label: 'Delete Admin', onClick: () => openModal('delete', admin), danger: true },
                            ] : [
                              { label: 'Edit Email', onClick: () => openModal('edit_email', admin) },
                              { label: 'Withdraw Invitation', onClick: () => openModal('withdraw', admin), danger: true },
                            ]}
                          />
                        )}
                      </td>
        ...
        <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-50 p-4">
          ...
          {modalType === 'edit_details' && (
            <div className="w-full max-w-sm bg-[#1c1c1c] border border-white/10 rounded-2xl p-6 shadow-2xl relative">
              ...
              <p className="text-xs text-zinc-400 mt-1 mb-6">Update name and phone number for {activeAdmin?.email}.</p>
          ...
          {modalType === 'delete' && (
            <div className="w-full max-w-sm bg-[#1c1c1c] border border-white/10 rounded-2xl p-6 shadow-2xl relative">
              <h3 className="text-base font-semibold text-white m-0">Delete Admin</h3>
              <p className="text-xs text-zinc-400 mt-1.5 mb-6">
                This permanently deletes <strong className="text-white font-semibold">{activeAdmin?.full_name || activeAdmin?.email}</strong> from the platform. Their sign-in, profile, and access are all removed. This action cannot be undone.
              </p>

              {currentUser?.id === activeAdmin?.id && (
          ...
          {modalType === 'withdraw' && (
            <div className="w-full max-w-sm bg-[#1c1c1c] border border-white/10 rounded-2xl p-6 shadow-2xl relative">
              ...
              <p className="text-xs text-zinc-400 mt-1.5 mb-6">
                Are you sure you want to withdraw the invitation for <strong className="text-white font-semibold">{activeAdmin?.email}</strong>? This email will no longer be whitelisted.
              </p>
