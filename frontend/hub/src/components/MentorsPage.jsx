...
export default function MentorsPage() {
  ...
  const fetchMentors = async () => {
    ...
    setError('');
    try {
      const response = await api.get('/api/mentors/?all=true');
      setMentors(response.data.mentors || []);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to fetch mentor accounts.');
    } finally {
      setLoading(false);
    }
  };

  const openModal = (type, mentor) => {
    setModalType(type);
    ...
    if (type === 'edit_details') {
      setFullNameVal(mentor.full_name || '');
      setMobileNumberVal(mentor.mobile_number || '');
    } else if (type === 'edit_email') {
      setEditEmailVal(mentor.email);
    }
  };
  ...
  const handleEditDetails = async (e) => {
    ...
    setModalError('');
    try {
      await api.patch(`/api/users/${activeMentor.id}/`, {
      ...
      setModalError(err.response?.data?.message || 'Failed to update mentor details.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteUser = async () => {
    ...
    setModalError('');
    try {
      await api.delete(`/api/users/${activeMentor.id}/`);
      fetchMentors();
      closeModal();
    } catch (err) {
              ...
              {filteredMentors.length === 0 ? (
                <tr>
                  <td colSpan="100%" className="py-8 text-center text-zinc-500 text-sm">
                    No mentor accounts found matching search criteria.
                  </td>
                </tr>
              ) : (
                getSortedData(filteredMentors).map(mentor => {
                  const initials = mentor.full_name?.split(' ').map(n => n[0]).join('').substring(0, 2) || 'M';
                  return (
                    <tr
                      key={mentor.id}
                    ...
                    >
                      ...
                      {visibleColumns.full_name && (
                        <td className="py-2 px-6 font-medium text-white text-sm align-middle whitespace-nowrap">
                          {mentor.kind === 'ghost' ? (
                            ...
                            mentor.full_name || <span className="text-zinc-500">—</span>
                          )}
                        </td>
                      )}
                      {visibleColumns.email && (
                        <td className="py-2 px-6 text-white text-sm align-middle whitespace-nowrap">{mentor.email}</td>
                      )}
                      {visibleColumns.mobile_number && (
                        <td className="py-2 px-6 text-[#a1a1aa] text-sm align-middle whitespace-nowrap">
                          {mentor.kind === 'ghost' ? '—' : (mentor.mobile_number || '—')}
                        </td>
                      )}
                      {visibleColumns.created_at && (
                        <td className="py-2 px-6 text-[#a1a1aa] text-sm align-middle whitespace-nowrap">
                          {mentor.kind === 'ghost' ? '—' : new Date(mentor.created_at).toLocaleDateString('en-US', {
                      ...
                      {visibleColumns.invited_by && (
                        <td className="py-2 px-6 text-zinc-300 text-sm align-middle whitespace-nowrap">
                          {mentor.invited_by_profile?.full_name || mentor.invited_by_profile?.email || '—'}
                        </td>
                      )}
                      {visibleColumns.students_count && (
                        <td className="py-2 px-6 text-zinc-300 text-sm align-middle text-center font-mono tabular-nums">
                          {mentor.kind === 'ghost' ? '—' : (mentor.assigned_students_count ?? 0)}
                        </td>
                      )}
                      <td className="py-2 px-2 align-middle text-right sticky right-0 bg-[#0a0a0a] group-hover:bg-[#111] border-l border-[rgba(255,255,255,0.08)] transition-colors z-10">
                        {currentUser?.role === 'ADMIN' && (
                          <StaffActionsDropdown
                            items={mentor.kind === 'active' ? [
                              { label: 'Edit Details', onClick: () => openModal('edit_details', mentor) },
                              { label: 'Delete Mentor', onClick: () => openModal('delete', mentor), danger: true },
                            ] : [
                              { label: 'Edit Email', onClick: () => openModal('edit_email', mentor) },
                              { label: 'Withdraw Invitation', onClick: () => openModal('withdraw', mentor), danger: true },
                            ]}
                          />
                        )}
                      </td>
        ...
        <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-50 p-4">
          ...
          {modalType === 'edit_details' && (
            <div className="w-full max-w-sm bg-[#1c1c1c] border border-white/10 rounded-2xl p-6 shadow-2xl relative">
              ...
              <p className="text-xs text-zinc-400 mt-1 mb-6">Update name and phone number for {activeMentor?.email}.</p>
          ...
          {modalType === 'delete' && (
            <div className="w-full max-w-sm bg-[#1c1c1c] border border-white/10 rounded-2xl p-6 shadow-2xl relative">
              <h3 className="text-base font-semibold text-white m-0">Delete Mentor</h3>
              <p className="text-xs text-zinc-400 mt-1.5 mb-4">
                This permanently deletes <strong className="text-white font-semibold">{activeMentor?.full_name || activeMentor?.email}</strong> from the platform. Their sign-in, profile, and access are all removed. This action cannot be undone.
              </p>

              {activeMentor?.assigned_students_count > 0 && (
                <div className="bg-zinc-900 text-zinc-400 rounded-md p-3.5 border border-white/5 text-xs mb-4 leading-normal">
                  Heads up: this mentor is currently assigned to <strong className="text-white font-semibold">{activeMentor.assigned_students_count} {activeMentor.assigned_students_count === 1 ? 'student' : 'students'}</strong>. Those students will lose their mentor assignment until you reassign them.
                </div>
              )}
          ...
          {modalType === 'withdraw' && (
            <div className="w-full max-w-sm bg-[#1c1c1c] border border-white/10 rounded-2xl p-6 shadow-2xl relative">
              ...
              <p className="text-xs text-zinc-400 mt-1.5 mb-6">
                Are you sure you want to withdraw the invitation for <strong className="text-white font-semibold">{activeMentor?.email}</strong>? This email will no longer be whitelisted.
              </p>
